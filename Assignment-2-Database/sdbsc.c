...
int get_student(int fd, int id, student_t *s)
{
    ...
    if (read(fd, &found_student, sizeof(student_t)) == -1)
    {
        return ERR_DB_FILE;
    }

    if (memcmp(&found_student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0)
    {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    if (read(fd, &existing_student, sizeof(student_t)) == -1)
    {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
